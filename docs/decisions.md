...
| 2026-09-21 | Keycloak escuta na 8081 dentro e fora da rede | Publicado como `8081:8080`, o `iss` do token seria alcançável de um lado só, e todo token minted do host seria recusado pela aplicação em container. Mesma porta dos dois lados resolve em vez de contornar. |
| 2026-09-21 | Imagem final `alpine`, não distroless | Distroless seria menor e com menos superfície, e não tem shell: o healthcheck precisaria de um binário próprio e ninguém entraria no container para olhar. Num projeto para ser lido e rodado à mão, inspecionar vale mais. |
| 2026-09-21 | `Claim` do registro de entrada usa `ON CONFLICT DO NOTHING` | Erro dentro de transação a aborta (`25P02`), e o passo seguinte do consumidor é ler o registro guardado. Detectar a duplicata sem levantar erro é o que mantém a transação utilizável. |
